...
def run(args):
    ...
    if args['conta'] > 0:
        datasize = positive.shape[0]
        contanumber = int(args['conta']*datasize)
        positive = positive[:datasize-contanumber]
        conta,_ = cifar10.get_test(label=args['inClass'],ratio_outliers=1.0)
        positive = np.concatenate((positive,conta[:contanumber]))
        shuffle_idx = np.arange(datasize)

...
def run(args):
    if 'cuda' in args:
        DEVICE = th.device("cuda:%d" % args['cuda'] if th.cuda.is_available() else "cpu")
    else:
        DEVICE = th.device("cuda" if th.cuda.is_available() else "cpu")
    seed = args['seed']
    np.random.seed(seed)
    ...
    unlabeled = th.FloatTensor(unlabeled.reshape(-1,1,28,28)).cuda().to(DEVICE)  if th.cuda.is_available() else th.FloatTensor(unlabeled.reshape(-1,1,28,28))
    ...
    positive = th.FloatTensor(positive.reshape(-1,1,28,28)).cuda().to(DEVICE) if th.cuda.is_available() else th.FloatTensor(positive.reshape(-1,1,28,28))
    ...
    encoder = WAE_Encoder(args).cuda().to(DEVICE) if th.cuda.is_available() else WAE_Encoder(args)
    decoderI = WAE_Decoder(args).cuda().to(DEVICE) if th.cuda.is_available() else WAE_Decoder(args)
    decoderO = WAE_Decoder(args).cuda().to(DEVICE) if th.cuda.is_available() else WAE_Decoder(args)
    discriminator = Discriminator(args).cuda().to(DEVICE) if th.cuda.is_available() else Discriminator(args)
    ...
    values = []
    advantageI = th.tensor([0], dtype=th.float).cuda().to(DEVICE) if th.cuda.is_available() else th.tensor([0], dtype=th.float)
    for epoch in range(args['epochs']):
        ...
        for cnt, x in enumerate(zip(positive_loader, unlabeled_loader)):
            ...
            if args['wae'] > 0:
                ...
                z = (th.randn(dset1.size()[0], args['n_z']) * args['sigma']).cuda().to(DEVICE) if th.cuda.is_available() else (th.randn(dset1.size()[0], args['n_z']) * args['sigma'])
                d_z = discriminator(z)
            ...
            lossUOs.extend(list(lossUO.detach().cpu().numpy()))
        ...
        lossUOs = np.array(lossUOs)
        lossIs = np.array(lossIs)

        epoch_data = args
        epoch_data['epoch'] = epoch
        epoch_data['precision'] = precision
        epoch_data['recall'] = recall
        epoch_data['F1'] = F1
        epoch_data['AUC'] = AUC
        epoch_data['AUPRC'] = AUPRC
        epoch_data['tn'] = tn
        epoch_data['tp'] = tp
        epoch_data['fn'] = fn
        epoch_data['fp'] = fp
        epoch_data['AP'] = average_precision_score(unlabeled_labels, scoreI)
        epoch_data['recon_error_Gi_pred'] = lossUIs[y_pred==0].mean() if y_pred.sum() > 0 else np.nan
        epoch_data['recon_error_Go_pred'] = lossUOs[y_pred==1].mean() if y_pred.sum() > 0 else np.nan
        epoch_data['recon_error_Gi_true'] = lossUIs[unlabeled_labels==0].mean()
        epoch_data['recon_error_Go_true'] = lossUOs[unlabeled_labels==1].mean()
